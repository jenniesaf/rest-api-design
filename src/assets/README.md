# Assets

This folder contains static assets like images, fonts, and other media files.

**Purpose:** Domain-agnostic static resources that can be used by any feature or app layer.

**Dependency Rules:** This is part of the Shared Layer. It CANNOT import from Features or App layers.

For more details on folder structure and rules, see [README.md](../../README.md).
